...
function testEverySome() {
    ...
    // 일부 객체의 age가 25세 초과 하는지 검증
    result = data.some(obj =>{
        return obj.age > 25
    })
    console.log("일부의 나이가 25세 초과인가?", result);
}
// testEverySome();
...
// mqp -> filter -> sort -> reduce 
function testMap() {
    // mqp -> 배열 형태는 그대로, 배열 요소를 변형
    console.log("========== map");
...
function testFilter() {
    console.log("======== filter");
    // filter: 내부요소는 그대로, 조건 만족하는 요소만 뽑아서 새 배열을 생성
...
function testReduce2(){
    ...
    let result = numbers.reduce((acc, value) => {
        console.log(console.log(`콜백 파라미터: (acc: ${acc}, value: ${value})`));
        acc.push(value * 2);
        console.log(` -> ${acc}`);
...
function testDataPipeline() {
    ...
    const studentsWithTotal = data.map(s => ({ // { } <- 배열블락  ()한번 더 씌워서 데이터만 추가 ...sutdent로 위 배열 한줄을  student로 표시
        ...s, total : s.kor + s.eng + s.math
    }))
    console.log("map:", studentsWithTotal);

    const filteredStudents = studentsWithTotal.filter(student => student.total >= 240)
    console.log("총점이 240이상:", filteredStudents);
    console.log("총점이 240 이상", studentsWithTotal.filter(student => student.total >= 240))
   
    // sort 함수 이용 정률
    const sortedStudents = filteredStudents.sort(
        (a,b) => b.total - a.total
    )
    console.log("총점 순 정렬:", sortedStudents)

    // reduce 함수 활용

    const totalSum = sortedStudents.reduce((acc, student) => acc + student.total, 0 );
    console.log("총점240 이상 학생들의 총점:", totalSum);
    const avgTotalSum = totalSum / sortedStudents.length;
    console.log("240이상 학생들의 평균점수:", avgTotalSum);
}
// testDataPipeline();
